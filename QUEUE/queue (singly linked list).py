...
class Q: #Q class 정의
    ...
    def __str__(self): #queue의 모든 data들을 출력하는 스페셜 메서드
        ...
        if self.is_empty():
            display += "empty queue\n"
        else: #비어있지 않으면 head와 tail이 가리키는 data도 출력하도록 추가 (동적할당이므로, index를 나타내는 것보다 유용한 정보라고 판단)
            display += "(head: {0} & tail: {1})\n".format(self.head.data, self.tail.data)
...
print("  P : Peek data of head                                                   \n")
print("  F : Check stack is full or not                                          \n")
print("  E : Check stack is empty or not                                         \n")
print("  # : Print the number of elements                			 \n")
print("  H : Print index of head	                                         \n")
print("  T : Print index of tail            				         \n")
print(" n- : (n: integer) Dequeue n times          		                 \n")
print(" ?@ : Check data @ is member of queue or not         		         \n")
print(" =@ : Replace data of tail to @	                                         \n")
print("  C : Delete all elements of queue                                        \n")
print("───────────────────────────────────────────────────\n")
print("\t      N  E  W   F U N C T I O N S					 \n")
print("───────────────────────────────────────────────────\n")
print(" $n : (n: integer) Change size of queue to n				 \n")
print("  D : Make queue be doubled				                 \n")
print("	   ex) a b c -> a a b b c c	 		   	                 \n")
print(" X@ : Enqueue @ until queue is full 	     		                 \n")
print("	   ex) a b c (if max == 10)	         			         \n")
print("	       >> Xh                     				         \n")
print("	       a b c h h h h h h h       				         \n")
print("───────────────────────────────────────────────────\n")
...
answer = []
MAX = int(input("  decide max size of queue: ")) #queue의 최대 크기 입력받기
...
while True:
    ...
    while i < long: #command가 남지 않을 때까지 반복
        ...
        elif answer[i] == '$': #queue의 최대 크기를 바꾸고 싶을 때
        ...
        elif answer[i] == 'X': #queue가 full해질 때까지 특정 data를 enqueue하고 싶을 때 
