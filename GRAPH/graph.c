...
int find_path_1(GRAPH*, VERTEX*, VERTEX*, char[]);
void find_path_2(GRAPH*, VERTEX*, VERTEX*);
	...
	while (signal) {
		...
		for (int i = 0; answer[i] != '\0'; i++) {
			if (answer[i] == 'G') {
				...
				add_edge(MG, answer[i + 2], answer[i + 4]);
				i += 5;
			}
			else if (answer[i] == 'L') {
				continue;
...
int find_path_1(GRAPH* MG, VERTEX* d1, VERTEX* d2, char back[]) {
	if (d1 == d2) {
		return 1;
	}
	...
	while (tmp) {
		if (!tmp->vertex->visited) {
			back[(tmp->vertex->data - 'A')] = d1->data;
			if (find_path_1(MG, tmp->vertex, d2, back))
				return 1;
		}
		tmp = tmp->more;
	}
...
void find_path_2(GRAPH* MG, VERTEX* d1, VERTEX* d2) {
	char back[100];
	char path[100];

	find_path_1(MG, d1, d2, back);
