...
class GRAPH:
    ...
    def delete_vertex(self, data):
        ...
        if target.connect == None:
            ...
            A = int(input("  Target has Edge, 1. Ignore and Delete  2. Don't Delete\n  : "))

            if A == 2:
                return
            else:
                dt = target.connect
                print("  Deleted Edge: (", end='')
                print("{}".format(dt.vertex.data), end='')
                for i in range(1, target.count_edge, 1):
                    dt = dt.more
                    print(",{}".format(dt.vertex.data), end='')
                print(")\n",end='')
...
print("          * (If Path Exist, Print Path)						")
...
print("  Please create Graph by command 'G' first")
answer = []
signal = 1
MG = GRAPH()
